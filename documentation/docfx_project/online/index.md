...
2.  オンライン環境にアップロード    
	...
	* 正常に終了した場合は、Scoring completeと表示され、result.jsonがダウンロードできるようになり、距離点・タイムがランキングに掲示されます。
	* 正常にシナリオ実行が終了しても、launchに失敗した等でスコアが出力されていない場合はNo resultとなり、最終的なタイムとしては使われません。
